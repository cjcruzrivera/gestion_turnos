...
def next_turno(request):
    ...
    turnos = Turno.objects.filter(isAtendido=False, hora_fin_turno__isnull=True, cajero__isnull=True, servicio_id=servicio.codigo).order_by('hora_solicitud')

    if not turnos:
        turnos = Turno.objects.filter(isAtendido=False, hora_fin_turno__isnull=True, cajero__isnull=True).order_by('hora_solicitud')
        if not turnos:
...
def AtenderTurnoView(request, turno):
    form = ClienteForm()
    return render(request, 'cajero/atender_turno.html', {'form':form})
